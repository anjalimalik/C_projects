...
int main() {
    ...
    // allocate memory for u & v float vectors.
    u = (float *)malloc(sizeof(float)*(vecsize));
    v = (float *)malloc(sizeof(float)*(vecsize));
    ...
    // Free all allocated memory to the float vectors
    free(x);
    free(y);
    free(u);
    free(v);
